...
const Car = require("../modules/cars/Car");
...
exports.updateCar = async (req, res) => {
  try {
    ...
    if (name !== undefined) {
      if (!String(name).trim()) {
        return res.status(400).json({ ok: false, message: "name bo'sh bo'lmasin" });
      }
    ...
    if (model !== undefined) {
      if (!String(model).trim()) {
        return res.status(400).json({ ok: false, message: "model bo'sh bo'lmasin" });
      }
